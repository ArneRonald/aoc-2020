...
function solveDayFour() {
    ...
    for (var i = 0; i < input.length; i++){
        ...
        var currentInput = input[i].split(" ")
        for (j = 0; j < currentInput.length; j++){
            var splitInput = currentInput[j].split(":")  
            switch (splitInput[0]){
                case "byr":
                    currentID.byr = splitInput[1];
                break
                case "iyr":
                    currentID.iyr = splitInput[1];
                break
                case "eyr":
                    currentID.eyr = splitInput[1];
                break
                case "hgt":
                    currentID.hgt = splitInput[1];
                break
                case "hcl":
                    currentID.hcl = splitInput[1];
                break
                case "ecl":
                    currentID.ecl = splitInput[1];
                break
                case "pid":
                    currentID.pid = splitInput[1];
                break
                case "cid":
                    currentID.cid = splitInput[1];
                break
            }
        }
    ...
    for (j = 0; j < results.length; j++){
        if ((results[j].byr == "") ||
            ...
            (results[j].ecl == "") ||
            (results[j].pid == "")){
            // console.log("Invalid: ",  results[j])
        } else if (validateBirthYear(results[j].byr) &&
                   validateIssueYear(results[j].iyr) && 
                   validateExpirationYear(results[j].eyr) && 
                   validateHeight(results[j].hgt) && 
                   validateHairColor(results[j].hcl) && 
                   validateEyeColor(results[j].ecl) &&
                   validatePassportNumber(results[j].pid))                 
                   {
            // console.log("valid: ", results[j])
            count++
        }
    }
    console.log(count)
...
function validateBirthYear(birthYear){
    if(birthYear.length == 4 && birthYear >= 1920 && birthYear <= 2020) {
        console.log("valid birth year", birthYear)
        return true
    } 
    return false;
}
...
function validateIssueYear(issueYear) {
    if (issueYear.length == 4 && issueYear >= 2010 && issueYear <= 2020) {
        console.log("valid issue year", issueYear)
        return true
    }
    return false
}
...
function validateExpirationYear(expirationYear) {
    if (expirationYear.length == 4 && expirationYear >= 2020 && expirationYear <= 2030) {
        console.log("valid epriration year", expirationYear)
        return true
    }
    return false
}
...
function validateHeight(height) {
    if (height.includes("in") || height.includes("cm")){
        if (height.includes("cm")){
            height = height.replace("cm", "")       
            if(height >= 150 && height <= 193 ) {
                console.log("valid height", height)
                return true
            }
        }
...
function validateHairColor(haircolor){
    var reg = new RegExp("[0-9a-f]")
    if (haircolor.charAt(0) == "#" && (haircolor.match("#") || []).length == 1) {
        var replaced = haircolor.replace("#", "")
        if (replaced.length == 6 && reg.exec(replaced)){
            console.log("valid hair", haircolor)
            return true
        }
    }
    return false
...
function validateEyeColor(eyeColor){
    switch (eyeColor){
        ...
        case "hzl" :
        case "oth" :
            console.log("valid eye color", eyeColor)
            return true;
        default:
            return false
    }
...
function validatePassportNumber(passportNumber){
    ...
    if (passportNumber.length == 9 && reg.exec(passportNumber)){
        console.log("valid passport", passportNumber)
        return true
    }
    return false
}